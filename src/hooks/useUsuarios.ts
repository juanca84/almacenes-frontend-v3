...
export function useUsuarios(): UseUsuariosReturn {
  ...
  const {
    items: usuarios, loading, total, totalPaginas,
    ...
    queryKey: ({ pagina, limite, sortBy, sortDir }) =>
      ['usuarios', 'list', { pagina, limite, filtro: debouncedFiltro, roles, estados, sortBy, sortDir }],
  ...
  const setFiltro  = (v: string)   => { _setFiltro(v);  setPagina(1) }
  const setRoles   = (v: string[]) => { _setRoles(v);   setPagina(1) }
  ...
  const exportarCSV = async () => {
    ...
    const headers = ['Usuario', 'Nombres', 'Primer apellido', 'Segundo apellido', 'Correo', 'Teléfono', 'Tipo documento', 'Nro. documento', 'Estado', 'Roles']
  ...
  return {
    ...
    pagina, limite, filtro, roles, estados, sortBy, sortDir,
